/*
 * Created by Mahmoud Aly - engma7moud3ly@gmail.com
 * Project Micro REPL - https://github.com/Ma7moud3ly/micro-repl
 * Copyright (c) 2023 . MIT license.
 *
 */

package micro.repl.ma7moud3ly.platform

import android.annotation.SuppressLint
import android.app.Activity
import android.content.pm.ActivityInfo
import android.content.res.Configuration
import androidx.activity.compose.LocalActivity
import androidx.compose.runtime.Composable
import androidx.compose.runtime.remember
import androidx.compose.ui.platform.LocalConfiguration
import micro.repl.ma7moud3ly.managers.port.AppManager

/**
 * [AppManager] backed by the hosting Activity.
 *
 * Rebuilt whenever the configuration changes, so [AppManager.isPortrait] is read
 * fresh after every rotation instead of being captured once.
 */
@Composable
fun rememberAppManager(): AppManager {
    val activity = LocalActivity.current
    val orientation = LocalConfiguration.current.orientation
    return remember(activity, orientation) {
        AndroidAppManager(
            activity = activity,
            isPortrait = orientation == Configuration.ORIENTATION_PORTRAIT
        )
    }
}

@SuppressLint("SourceLockedOrientationActivity")
private class AndroidAppManager(
    private val activity: Activity?,
    override val isPortrait: Boolean
) : AppManager {

    override fun toggleOrientation() {
        activity?.requestedOrientation = if (isPortrait) {
            ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
        } else {
            ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
        }
    }

    override fun forcePortrait() {
        activity?.requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
    }

    override fun restart() {
        activity?.recreate()
    }
}
